...
class MiniGPTBase(BaseModel):
    ...
    def __init__(
        ...
        lora_r=0,# lora_r=0 means lora is not used
        lora_alpha=0,
        vit_lora_r=0,
        vit_lora_alpha=0,
        lora_dropout=0.05,
        ...
        self.visual_encoder, self.ln_vision = self.init_vision_encoder(
            vit_model, img_size, drop_path_rate, use_grad_checkpoint, vit_precision, freeze_vit, vit_lora_r, vit_lora_alpha
        )
        ...
        self.prompt_list = []
        self.loss_con = nn.CrossEntropyLoss(reduction='sum')
    ...
    def preparing_embedding(self, samples):
        ...
        if 'image' in samples and 'mask' not in samples:
            img_embeds, img_atts = self.encode_img(samples["image"])
        elif "image_path" in samples and 'mask' not in samples:
            img_embeds, img_atts = self.encode_sam_out(samples["image_path"])
    ...
    def generate(
        ...
        img_embeds, atts_img = self.encode_sam_out(image_path)
        ...
        for output_token in outputs:
            ...
            output_texts = output_texts.split(self.end_sym)[0]
            output_texts = output_texts.replace("<s>", "")
